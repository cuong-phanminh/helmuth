...
/*
  Plugin Name: Seadev Purge Varnish
...
class Purge_Varnish {
    ...
    function purge_varnish_add_menu() {
        add_menu_page('Purge Varnish', 'Purge Varnish', 'manage_options', 'purge-varnish-all', array($this, 'purge_varnish_page_file_path'), '', '2.2.9');

        if (current_user_can('manage_options')) {
            add_submenu_page('purge-varnish-all', 'Settings', 'Settings', 'manage_options', 'purge-varnish-settings', array(
    ...
    function purge_varnish_uninstall() {
        ...
        delete_option('clear_after_update');
    }
...
/**
 * Clear varnish cache after save post
 */
function seadev_purge_varnish_after_save_post() {
    $clear_varnish_after_update = get_option('clear_varnish_after_update', '');
    if ($clear_varnish_after_update == 1) {
        $purge_varnish = new Purge_Varnish();
        $purge_varnish->purge_varnish_all_cache_manually();
    }
}
add_action( 'save_post', 'seadev_purge_varnish_after_save_post' );