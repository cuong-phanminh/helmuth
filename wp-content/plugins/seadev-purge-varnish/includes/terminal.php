...
if (isset($_POST['savecode'])) {
    if ($purge_varnish->purge_varnish_nonce('pvTsetting') == true) {
        ...
        update_option('clear_varnish_after_update', sanitize_text_field($_POST['clear_varnish_after_update']));
...
$clear_varnish_after_update = esc_html(get_option('clear_varnish_after_update', ''));
if (esc_html(get_option('varnish_control_terminal', ''))) {
    $varnish_control_terminal = esc_html(get_option('varnish_control_terminal', ''));
}else{
    $varnish_control_terminal = "127.0.0.1:6082";
}
if (esc_html(get_option('varnish_control_key', ''))) {
    $varnish_control_key = esc_html(get_option('varnish_control_key', ''));
}else{
    $varnish_control_key = "caa24c8a-cb50-4a27-9e4b-8445679f5472";
}
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            <span class="terminal_desc"><?php esc_html_e('For debug add the above constant in wp-config.php file'); ?><span>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select name="varnish_bantype" class="form-select">
                                                <option value="1" <?php
                                                if ($varnish_bantype == '1') {
                                                    echo 'selected="selected"';
                                                }
                                                ?>>Ban Lurker</option>
                                                <option value="0" <?php
                                                if ($varnish_bantype == '0') {
                                                    echo 'selected="selected"';
                                                }
                                                ?>>Normal</option>
                                            </select>
                                    ...
                                    <tr>
                                        <th valign="top"><?php esc_html_e('Clear cache after update post:') ?> </th>
                                        <td>
                                            <select name="clear_varnish_after_update" class="form-select">
                                                <option value="0" <?php
                                                if ($clear_varnish_after_update == '0') {
                                                    echo 'selected="selected"';
                                                }
                                                ?>>No</option>
                                                <option value="1" <?php
                                                if ($clear_varnish_after_update == '1') {
                                                    echo 'selected="selected"';
                                                }
                                                ?>>Yes</option>
                                            </select>
